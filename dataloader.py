...
def get_train_loader(cfg):
    ...
    train_transform = transforms.Compose([Rescale((1025, 2049)),
                                          RandomChannel(),
                                          ToTensor(),
                                          Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225],
                                                    max_depth=80)])

    train_set = DepthData(cfg['data']['train_split'], transform=train_transform)
...
def get_valid_loader(cfg):
    # TODO channel变换后与Normalization的值对应不上
    valid_transform = transforms.Compose([Rescale((1025, 2049)),
                                         ToTensor(),
                                         Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225],
                                                   max_depth=80)])

    valid_set = DepthData(cfg['data']['valid_split'], transform=valid_transform)
...
def get_test_loader(cfg):
    ...
    valid_set = DepthData(cfg['data']['test_split'], transform=valid_transform)
...
if __name__ == '__main__':
    ...
    with open("config/icnet-depth.yaml") as fp:
        cfg = yaml.load(fp)
    for sample in get_train_loader(cfg):
